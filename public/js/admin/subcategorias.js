...
const cargarSubcategorias = async (page = 1) => {
    try {
        const response = await axios.get(`/admin/subcategorias?page=${page}`);
        ...
        mostrarError("Error al cargar las subcategorías");
    }
};
...
const eliminarSubcategoria = async (id) => {
    ...
    try {
        await axios.delete(`/admin/subcategorias/${id}`);
        await cargarSubcategorias(currentPage);
        mostrarExito("Subcategoría eliminada correctamente");
    } catch (error) {
        mostrarError("Error al eliminar la subcategoría");
    }
};

// Funciones de renderizado
const renderizarTabla = () => {
    const tbody = document.querySelector("#subcategoriasTable tbody");
    tbody.innerHTML = subcategorias
        .map(
            (subcategoria) => `
        <tr class="hover:bg-[#F3C71E] hover:bg-opacity-5 transition-colors">
            ...
            <td class="px-6 py-4 text-center">
                <div class="flex justify-center space-x-3">
                    <a href="/admin/subcategorias/${subcategoria.id}/edit" 
