...
<div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
    <div class="p-4 sm:p-6 text-gray-900 dark:text-gray-100">
        <div class="mb-6 sm:mb-8">
            <h1 class="text-2xl sm:text-3xl font-bold text-[#1A323E] dark:text-white mb-2">Editar Descuento</h1>
        ...
        <div class="max-w-2xl mx-auto">
            <form id="descuentoForm" action="{{ route('admin.descuentos.update', $descuento) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 border border-[#F3C71E] border-opacity-20">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="descripcion" 
                                  ...
                                  rows="3"
                                  class="w-full px-4 py-2 rounded-lg border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-[#1A323E] dark:text-white focus:border-[#F3C71E] focus:ring-[#F3C71E] transition-colors"
                                  placeholder="Describe el descuento...">{{ $descuento->descripcion }}</textarea>
                        <div class="error-message text-red-600 dark:text-red-400 text-sm mt-2 hidden"></div>
                    </div>
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const productosSeleccionados = @json($productosSeleccionados);
    ...
    try {
        ...
        container.innerHTML = data.data.map(producto => `
            <div class="flex items-center space-x-3 p-2 rounded-lg hover:bg-[#F3C71E] hover:bg-opacity-10 transition-colors">
                <input type="checkbox" 
                       ...
                       ${productosSeleccionados.includes(producto.id) ? 'checked' : ''}
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (!response.ok) {
                    throw new Error(result.message || 'Error al actualizar el descuento');
                }

                showAlert('Descuento actualizado exitosamente', 'success');
